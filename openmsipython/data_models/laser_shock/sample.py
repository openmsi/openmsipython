...
class LaserShockSampleSpec(SpecForRun):
    ...
    def get_spec_kwargs(self):
        ...
        spec_kwargs[
            "notes"
        ] = "One of the possible Specs for producing Samples for the Laser Shock Lab"
        # process
    ...
    def __get_process(self):
            ...
            if self.preproc_temp is not None and self.preproc_temp != "":
                ...
                preprocessing.parameters.append(
                    Parameter(
                        ...
                        value=NominalReal(
                            float(self.preproc_temp), temp.bounds.default_units
                        ),
                        template=temp,
                        origin="specified",
                    )
        ...
        if (
            self.mat_proc is not None
            and self.mat_proc != ""
            and len(self.mat_proc.split("\r")) > 1
        ):
        ...
        for ipt, pt in enumerate(proc_temps, start=1):
            if pt is not None and pt != "":
                sample_processing.conditions.append(
                    Condition(
                        name=f"ProcessingTemperature{ipt}",
                        value=NominalReal(
                            float(pt),
                            self.templates.attr(
        ...
        if self.proc_time != "":
            sample_processing.parameters.append(
                Parameter(
                    name="ProcessingTime",
                    value=NominalReal(
                        float(self.proc_time),
                        self.templates.attr("Processing Time").bounds.default_units,
                    ),
        ...
        if (self.ann_temp is not None and self.ann_temp != "") and (
            self.ann_time is not None and self.ann_time != ""
        ):
