...
from openmsipython.data_models.utilities import (
    get_tag_value_from_list,
    get_json_filename_for_gemd_object,
)
from openmsipython.data_models.cached_isinstance_functions import (
    cached_isinstance_generator,
)
from openmsipython.data_models.cached_isinstance_functions import (
    isinstance_template,
    isinstance_spec,
    isinstance_run,
    isinstance_material_run,
)
from openmsipython.data_models.cached_isinstance_functions import (
    isinstance_link_by_uid,
    isinstance_list_or_tuple,
    isinstance_dict_serializable,
)
from openmsipython.data_models.gemd_template_store import GEMDTemplateStore
from openmsipython.data_models.gemd_spec_store import GEMDSpecStore
from openmsipython.data_models.spec_from_filemaker_record import SpecFromFileMakerRecord
...
isinstance_spec_from_filemaker_record = cached_isinstance_generator(
    SpecFromFileMakerRecord
)
isinstance_run_from_filemaker_record = cached_isinstance_generator(
    RunFromFileMakerRecord
)
isinstance_material_run_from_filemaker_record = cached_isinstance_generator(
    MaterialRunFromFileMakerRecord
)
...
class LaserShockLab(DataFileDirectory):
        ...
        if self.__password is None:
            ...
            if self.__password == "$JHED_PWORD":
                self.__password = getpass.getpass(
                    f"Please enter the JHED password for {self.username}: "
                )
    ...
    def all_object_lists(self):
        return {
            LaserShockGlassID: self.glass_IDs,
    ...
    def __init__(
        self, dirpath=pathlib.Path("./gemd_data_model_dumps"), *args, **kwargs
    ):
        """
    ...
    def create_gemd_objects(self, records_dict=None):
        ...
        self.get_objects_from_records(
            LaserShockGlassID, self.glass_IDs, "Glass ID", **extra_kwargs
        )
        self.logger.debug(f"There are {len(self.glass_IDs)} Glass ID objects")
        self.get_objects_from_records(
            LaserShockFoilID, self.foil_IDs, "Foil ID", **extra_kwargs
        )
        self.logger.debug(f"There are {len(self.foil_IDs)} Foil ID objects")
        self.get_objects_from_records(
            LaserShockEpoxyID, self.epoxy_IDs, "Epoxy ID", **extra_kwargs
        )
        self.logger.debug(f"There are {len(self.epoxy_IDs)} Epoxy ID objects")
        self.get_objects_from_records(
            LaserShockSpacerID, self.spacer_IDs, "Spacer ID", **extra_kwargs
        )
        ...
        self.logger.debug(
            f"There are {len(self.flyer_cutting_programs)} Flyer Cutting Program objects"
        )
        self.get_objects_from_records(
        ...
        self.logger.debug(
            f"There are {len(self.spacer_cutting_programs)} Spacer Cutting Program objects"
        )
        # Flyer Stacks (Materials)
        self.logger.debug("Creating Flyer Stacks...")
        self.get_objects_from_records(
            LaserShockFlyerStack,
            self.flyer_stacks,
            "Flyer Stack",
            self.glass_IDs,
            self.foil_IDs,
        ...
        self.get_objects_from_records(
            LaserShockSample, self.samples, "Sample", **extra_kwargs
        )
        ...
        self.logger.debug(
            f"There are {len(self.launch_packages)} Launch Package objects"
        )
        # Experiments (Measurements)
        self.logger.debug("Creating Experiments...")
        self.get_objects_from_records(
            LaserShockExperiment,
            self.experiments,
            "Experiment",
            self.launch_packages,
            **extra_kwargs,
        ...
        if n_specs_removed > 0:
            self.logger.debug(
                f"Removed {n_specs_removed} unneeded Specs from the store"
            )
    ...
    def get_objects_from_records(
        self,
        obj_type,
        obj_list,
        layout_name,
        *args,
        n_max_records=100000,
        records_dict=None,
        **kwargs,
    ):
        """
        ...
        if n_recs_skipped > 0:
            self.logger.info(
                f"Skipped {n_recs_skipped} {layout_name} records that already exist in {self.dirpath}"
            )
        ...
        for ir, record in enumerate(records_to_use):
            if ir > 0 and ir % 25 == 0:
                self.logger.debug(
                    f"\tCreating {obj_type.__name__} {ir} of {len(records_to_use)}..."
                )
            print(record)
            exit()
    ...
    def dump_to_json_files(
        ...
        for obj_type, obj_list in self.all_object_lists.items():
            objs_to_dump = (
                obj_list[: min(n_max_objs, len(obj_list))]
                if n_max_objs > 0
                else obj_list
            )
            for obj in objs_to_dump:
                ...
                if (
                    self.encoder.scope in obj.uids.keys()
                    and obj.uids[self.encoder.scope] in self.__uids_written
                ):
                    continue
                ...
                if complete_histories:
                    if isinstance_material_run(obj):
                        ...
                        with open(
                            self.dirpath
                            / fn.replace(".json", "_material_history.json"),
                            "w",
                        ) as fp:
            ...
            for uvn, uv in obj.unique_values.items():
                if uvn not in unique_vals.keys():
                    if io == 0:
                        ...
                        errmsg += (
                            f"name {uvn}! Recognized names are {unique_vals.keys()}"
                        )
                        self.logger.error(errmsg, RuntimeError)
