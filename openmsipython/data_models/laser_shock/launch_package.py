...
from openmsipython.data_models.utilities import (
    search_for_single_name,
    search_for_single_tag,
)
    ...
    def __get_process(self):
        ...
        if self.spacercutting is not None:
            ...
            cutting_spacer = ProcessSpec(
                name=self.spacercuttingname
                if self.spacercuttingname != ""
                else "Unknown Spacer Cutting",
        ...
        if self.spacercuttingenergy != "":
            ...
            if laser_cutting_energy_par is not None:
                new_value = NominalReal(
                    float(self.spacercuttingenergy), temp.bounds.default_units
                )
                p.value = new_value
            else:
        ...
        n_passes_values = [
            self.spacercuttingnpasses_1,
            self.spacercuttingnpasses_2,
            self.spacercuttingnpasses_3,
        ]
        ...
        if (
            cutting_spacer != self.spacercutting
            and "ObjectType::LaserShockSpacerCuttingProgram" in cutting_spacer.tags
        ):
        ...
        IngredientSpec(
            name="Chosen Flyer",
            material=choosing_flyer.output_material,
            process=attaching_spacer,
        )
        IngredientSpec(
            name="Cut Spacer",
            material=cutting_spacer.output_material,
            process=attaching_spacer,
        )
        ...
        IngredientSpec(
            name="Flyer and Spacer",
            material=attaching_spacer.output_material,
            process=attaching_sample,
        )
        IngredientSpec(
            name="Impact Sample",
            material=self.impactsamplespec
...
class LaserShockLaunchPackage(MaterialRunFromFileMakerRecord):
    ...
    def __init__(
        ...
        flyer_stacks,
        spacer_IDs,
        spacer_cutting_programs,
        samples,
        **kwargs,
    ):
        # find the flyer stack, spacer ID, spacer cutting program, and sample that were used
        self.flyerstack = search_for_single_tag(
            [fs for fs in flyer_stacks],
            "FlyerID",
            record.pop("Flyer ID").replace(" ", "_"),
        )
        self.spacerID = search_for_single_name(
            [sid for sid in spacer_IDs],
            record.pop("Spacer Type"),
            logger=kwargs.get("logger"),
        )
        self.spacercuttingname = record.pop("Spacer Cutting Program")
        self.spacercutting = search_for_single_name(
            [scp for scp in spacer_cutting_programs],
            self.spacercuttingname,
            logger=kwargs.get("logger"),
        )
        self.sample = search_for_single_name(
            [s for s in samples], record.pop("Sample Name"), logger=kwargs.get("logger")
        )
        # create Runs from Specs that were found
        self.spacer = (
            make_instance(self.spacerID) if self.spacerID is not None else None
        )
        # create the Impact Sample that was cut from the original sample
        self.impactsample = self.__get_impact_sample(
            record, kwargs.get("templates"), kwargs.get("specs")
        )
    ...
    def get_spec_kwargs(self, record):
        ...
        kwargs["flyerspec"] = (
            self.flyerstack.spec if self.flyerstack is not None else None
        )
        ...
        kwargs["impactsamplespec"] = (
            self.impactsample.spec if self.impactsample is not None else None
        )
        ...
        kwargs["samp_attachments_adhesives"] = record.pop(
    ...
    def __get_impact_sample(self, record, templates, specs):
        ...
        polish_proc = (
            record.pop("Polishing Process")
        ).rstrip()  # extra space at the end of some values in the DB
        ...
        ns = [
            ...
            "Polishing Diamond Grit 2",
            "Polishing Silicon Grit 2",
        ]
        ...
        for cp in cutting_procs:
            if cp == "":
                continue
            cutting_and_polishing.conditions.append(
                Condition(
                    name=n,
        ...
        for n, v, tn in zip(ns, vs, tns):
            if v not in ("", "N/A", "N/"):
                cutting_and_polishing.parameters.append(
                    Parameter(
                        name=n,
        ...
        IngredientSpec(
            name="Sample", material=self.sample.spec, process=cutting_and_polishing
        )
